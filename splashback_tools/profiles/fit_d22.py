...
def fit_profile(n_params, prior, prof, data_vec, base_path, n_live_points=500):

    def ln_likelihood(theta, r_data, dens_data, cov):
        dens = prof(theta, r_data)
        diff = dens - dens_data
...
def prior_d22(cube, ndim=9, nparam=9): #Change if chains aren't converging
...
fit_nd = partial(fit_profile, 9, prior_d22)
fit_3d = partial(fit_nd, rho)
fit_2d = partial(fit_nd, sigma)